    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title">Question Library</h2>
          ...
          <template v-if="loading">
            <div class="row align-items-center justify-content-center" style="min-height:600px">
              <div class="col-12 text-center">
                ...
                <p>Converting question, please wait...</p>
              </div>
            </div>
          </template>
          <template v-else>
            <template v-if="step == 'list'">
              <div class="row align-items-center mb-3">
                ...
                <div class="col-8 text-end">
                    <button class="btn btn-primary" @click="step = 'create'">Create Question</button>
                </div>
              </div>
              <div class="row">
                <data-table
                    :fields="fields"
                    :url="'/api/v1/question/data'"
                    :param="{
                      ...filter,
                    }"
                    :refresh="refresh"
                  ...
                  > 
                   <template #action="action">
                     <button class="btn btn-white" @click="selectQuestion(action.action.item)">
                      Select Question
                     </button>
                    </template>
                  </data-table>
              ...
              <div class="row">
                <div class="col-12">
                  <div class="d-flex align-items-center gap-3">
                    ...
                    <p class="mb-0 fw-bold">Create Question</p>
                  </div>
                  <hr>
                </div>
              </div>
              <div class="row">
                <form  @submit.prevent="(e) => submitDesign(e)" >
                  <div class="row align-items-top">
                    <div class="col-lg-4">
                      <p class="mb-0 fw-bold">Question <span class="text-danger">*</span></p>
                    </div>
                    <div class="col-lg-8">
                      <quill-editor :content="params.question"
                    ...
                    <div class="col-12">
                     ...
                     <div class="col-lg-4">
                      <p class="mb-0 fw-bold">Model Answer</p>
                    </div>
                    <div class="col-lg-8">
                      <textarea class="form-control" v-model="params.model_answer"></textarea>
                    </div>
                  </div>
                  <div class="row">
...
export function imageHandler () {
  ...
  input.onchange = async () => {
    ...
    try {
      console.info('before');
      await axios.post('/api/v1/question/upload-image',formData).then(res => {
        url = res.data.url
      })
    } catch (err) {
...
export default {
  ...
  methods:{
    ...
    async submitDesign(form){
      this.loading = true
      try {
        const response2 = await axios.post(`/api/v1/question/add`,this.params)
        this.$root.toast('Question Created!','success')
        this.loading = false
        if(this.formOnly){
          $('.modal').modal('hide')
        }else{
