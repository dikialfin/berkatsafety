          ...
          <template v-else>
            ...
            <template v-if="isEligible && !isFinish">
              <div class="text-left mb-5">
                  ...
                  <h1 class="mb-0" style="font-size:28pt">Reset Password</h1>
             ...
             <template v-if="isEligible && isFinish">
              <div class="text-center">
                  ...
                  <div class="text-center mt-3">
                    <h2 style="line-height:1.5em">Congratulation! Your account password has been successfully reset.</h2>
                    <button class="btn btn-lg btn-primary px-4" @click="$router.push({path:'/'})">
                      Back to Home
                    </button>
                  </div>
...
export default {
  ...
  methods: {
    validateToken(){
      ...
      axios.get(`/api/v1/check-reset-token/${this.$route.params.token}`)
      .then(res => {
        console.log(res)
    ...
    submit() {
      ...
      this.errors = null
      axios.post('/api/v1/reset', this.data)
        .then((response) => {
