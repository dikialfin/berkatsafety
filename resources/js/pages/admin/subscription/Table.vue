      ...
      <div class="row d-flex align-items-center mt-2 mb-4">
        ...
        <div class="col-12 col-lg-6 mt-lg-0 mt-3">
          <div class="d-flex gap-2 d-md-flex justify-content-md-end" >
            <router-link :to="{name:'admin.subscription.add'}">
...
export default {
  ...
  methods: {
    edit (data) {
      this.$router.push({ name: 'admin.subscription.edit', params: { id: data.action.item.id } })
    },
    deleteData (data) {
      this.$store.dispatch('set_delete_data',{
        url:`/api/v1/subscription/delete/${data.action.item.id}`
      })
      $('#delete-modal').modal('show');
    },
    restoreData (data) {
      this.$store.dispatch('set_restore_data',{
        url:`/api/v1/subscription/restore/${data.action.item.id}`
      })
    ...
    changeStatus (e,data){
      ...
      this.$store.dispatch('set_status_data',{
        url:`/api/v1/subscription/${data.action.item.status == 0?'active':'deactive'}/${data.action.item.id}`
      })
