<template>
  <form @submit.prevent="save">
    <div class="row pb-4">
      <div class="col-12">
        <card-form :title="'Add Catalogue'">
          <div class="row align-items-center mt-4">
            <div class="col-lg-2 col-12">
              <label>Title <span class="text-danger">*</span></label>
            </div>
            <div class="col-lg-4  col-12">
              <input class="form-control" v-model="params.title" required>
            </div>
            <div class="col-lg-2 col-12">
          ...
          <div class="row align-items-center mt-4">
            ...
            <div class="col-lg-10 col-12">
              <select-2
                :api="'/api/v1/brands/select'"
                :valuemultiple="params.brand_id"
                :multiple="true"
                :required="true"
                :onchange="(val) => params.brand_id = val"
                :settings="{
                  multiple:true
                }"
              />
            ...
            <card-page>
              <div class="row">
                <div class="col-12 col-sm-6">
                  <div class="card">
                    <div class="card-header d-flex">
                      ...
                      <button type="button" class="btn btn-sm btn-primary d-inline-block" @click="openFileManagerImage">Open Media</button>
                    </div>
                    <div class="card-body">
                ...
                <div class="col-12 col-sm-6">
                  <div class="card">
                    <div class="card-header d-flex">
                      <h5>File Pdf <span class="text-danger">*</span></h5>
                      <button type="button" class="btn btn-sm btn-primary d-inline-block" @click="openFileManager">Open Media</button>
                    </div>
                    <div class="card-body">
                      <div v-if="params.previewPdf.length > 0">
                        <div class="row">
                          <template v-for="(item, index) in params.previewPdf" :key="index">
                            <div class="col-sm-7 mx-sm-auto col-12 border rounded " style="height: 300px;" v-if="index == 0">
                              <iframe 
                                :src="item" 
                                width="100%" 
        ...
        <div class="row">
            <div class="col-12">
              <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <button class="btn btn-primary" :disabled="loading || !params.previewImages || !params.previewPdf">
                  <b-spinner
                    v-if="loading"
...
export default {
  ...
  methods: {
    openFileManager() {
      const fileManagerUrl = '/media?type=file';
      ...
      const self = this;
      self.params.previewPdf = []
      window.SetUrl = function(url) {
        url.forEach((res) => {
          console.log(res);
            if (!self.params.previewPdf.some(image => image.url === res.url)) {
                self.params.previewPdf.push(res.url)
            }
        });  
      };
    },
    ...
    async getData () {
      await axios.get(`/api/v1/catalogue/edit/${this.$route.params.id}`).then(res => {
    ...
    async save () {
      ...
      this.loading = true
        await axios.post('/api/v1/catalogue/add', param).then(res => {
          this.$router.go(-1)
          this.loading = false
