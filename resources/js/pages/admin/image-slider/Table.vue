    ...
    <div class="col-12">
      <div class="row d-flex align-items-center mt-2 mb-4">
        <div class="col-12 col-lg-6">
           <h2 class="mb-0 ">
            <i class="fe fe-list bg-primary text-white p-3 me-2" style="border-radius:12px"></i> Image Slider
          </h2>
        </div>
        <div class="col-12 col-lg-6 mt-lg-0 mt-3">
          <div class="d-flex gap-2 d-md-flex justify-content-md-end" >
            <template v-if="user.permission.includes('content.create') || [1].includes(user.role_id)">
              <router-link :to="{name:'admin.slider.add'}">
...
export default {
      ...
      fields: [
        {
          ...
          formatter:function(val,row,item){
            let file = `<img src="'/images/default-img.png'}" class="avatar-img mr-1" style="width:100px;height:auto">` 
            if (item.product_media.length > 0) {
              file = `<img src="${item.product_media[0].value?item.product_media[0].value : '/images/default-img.png'}" class="avatar-img mr-1" style="width:100px;height:auto">`
              if (item.product_media[0] && item.product_media[0].type == 'file') {
                const parts = item.product_media[0].value.split("/");
                const fileName = parts[parts.length - 1];
                file = ` <a href="${item.product_media[0].value}" target="_blank" class="text-info">
                            <div style="width:100px;height:100px" class="border p-2 rounded pt-4 text-center">
                              <span>${fileName}</span>
                            </div>
                          </a>`
              }
  ...
  methods: {
    edit (data) {
      this.$router.push({ name: 'admin.products.edit', params: { id: data.action.item.id } })
    },
    deleteData (data) {
      this.$store.dispatch('set_delete_data',{
        url:`/api/v1/products/delete/${data.action.item.id}`
      })
      $('#delete-modal').modal('show');
    },
    restoreData (data) {
      this.$store.dispatch('set_restore_data',{
        url:`/api/v1/products/restore/${data.action.item.id}`
      })
