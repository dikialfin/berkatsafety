    ...
    <div class="col-12">
      <div class="row d-flex align-items-center mt-2 mb-4">
        <div class="col-12 col-lg-6">
           <h2 class="mb-0 ">
            <i class="fe fe-list bg-primary text-white p-3 me-2" style="border-radius:12px"></i> Categories
          </h2>
        </div>
...
export default {
    ...
    return {
      fields: [
          ...
          formatter:function(val,row,item){
            return `
              <div class="d-flex align-items-center">
                <div class="me-3">
                  <img src="${item.logo?item.logo : '/images/default-img.png'}" class="avatar-img mr-1" style="width:100px;height:auto">
                </div>
                <div>
        ...
        },
        {
          ...
          formatter:function(val,row,item){
            let child = []
            item.children.map(res => child.push(res.name))
            return item.children.length == 0 ? 'Child' : child.join(', ')
          }
        },
        {
  ...
  methods: {
    edit (data) {
      this.$router.push({ name: 'admin.categories.edit', params: { id: data.action.item.id } })
    },
    deleteData (data) {
      this.$store.dispatch('set_delete_data',{
        url:`/api/v1/categories/delete/${data.action.item.id}`
      })
      $('#delete-modal').modal('show');
    },
    restoreData (data) {
      this.$store.dispatch('set_restore_data',{
        url:`/api/v1/categories/restore/${data.action.item.id}`
      })
