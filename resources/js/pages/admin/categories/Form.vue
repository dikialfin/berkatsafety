        ...
        <card-form :title="'Categories'">
          <div class="row align-items-center">
            ...
            <div class="col-lg-4 col-12">
              <button type="button" class="btn btn-sm btn-primary d-inline-block" @click="openFileManagerImage">Upload Category Logo</button>
            </div>
          </div>
          ...
          <div class="row align-items-center mt-4">
            ...
            <div class="col-lg-4 col-12">
              <select-2
                :api="'/api/v1/categories/select'"
                :value="params.parent_id"
                :required="false"
                :select="({id,text}) => this.params.parent_id = id"
              />
            </div>
          </div>
        </card-form>
...
export default {
  ...
  methods: {
    ...
    async getData () {
      await axios.get(`/api/v1/categories/edit/${this.$route.params.id}`).then(res => {
    ...
    async save () {
      ...
      this.loading = true
      if (!isNaN(this.$route.params.id)) {
        await axios.post(`/api/v1/categories/update`, param).then(res => {
          this.$router.go(-1)
          this.loading = false
        ...
        await axios.post('/api/v1/categories/add', param).then(res => {
          this.$router.go(-1)
          this.loading = false
